...
beforeAll(async () => {
  ...
  const ord = db.prepare("INSERT INTO orders (total, discount, tax, payment_method, status) VALUES (?, 0, 0, ?, 'completed')")
  const o1 = Number(ord.run(105, 'cash').lastInsertRowid)
  const o2 = Number(ord.run(45, 'credit_card').lastInsertRowid)
...
describe('CsvExportService.formatAsCSV', () => {
  it('prepends a UTF-8 BOM and a single header row for empty input', () => {
    ...
    expect(csv.startsWith('\uFEFFOrder ID,Date/Time,Items,Total Amount,Payment Method')).toBe(true)
    expect(csv.replace('\uFEFF', '').split(/\r?\n/)).toHaveLength(1)
  })

  it('formats ISO datetime, 2-decimal totals, and quotes/escapes cells', () => {
    const orders: OrderExport[] = [
      { id: '1', datetime: '2026-09-17 10:30:00', items: '豆漿 (2), Milk, 2% (1)', total: 105, paymentMethod: 'cash' },
      { id: '2', datetime: '2026-09-17 11:45:00', items: 'Snack "Large" (1)', total: 45, paymentMethod: 'credit_card' },
    ]
    ...
    expect(lines[1]).toBe('1,2026-09-17T10:30:00,"豆漿 (2), Milk, 2% (1)",105.00,cash')
    expect(lines[2]).toBe('2,2026-09-17T11:45:00,"Snack ""Large"" (1)",45.00,credit_card')
  })
})